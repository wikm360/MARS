...
> **مهم:** برنامه `config.yaml` را از **کنار خودش** می‌خواند، نه از داخل exe.
> ساختار زیر را **دقیقاً** رعایت کن، وگرنه با خطای `FileNotFoundError` باز نمی‌شود.

```
هر پوشه‌ای روی سیستم هدف/
├── WindowsSecurityService.exe
└── client/
    └── config.yaml          ← فایل تنظیم‌شده با آی‌پی سرور و secret
```
